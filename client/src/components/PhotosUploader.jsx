...
function PhotosUploader({addedPhotos,onChange}) {
  ...
  // Remove Photo, onChange sets addedPhotos state to a new array of pictures from previous state exlcuding the selected photo
  function removePhoto(ev,filename){
    ev.preventDefault()
    onChange([...addedPhotos.filter(photo=>photo!==filename)])
  }

  // Select as Main Photo, onChange sets addedPhotos state to a new array of pictures with the selected photo as the first pic, appending the
  //remaining addedPhotos State 
      ...
      {addedPhotos.length>0 && addedPhotos.map(link=>(
          <div className="h-32 flex relative" key={link}>
            ...
            <button onClick={(ev)=>selectAsMainPhoto(ev,link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
              {/* If link is the same as the first picture, make it a solid star */}
              {link===addedPhotos[0] && (
